<script lang="ts">
    ...
    onMount(() => {
      ...
      if (isMac) {
        ...
        isAppleSilicon = /arm64|aarch64/i.test(userAgent);
      }
    });
  </script>
  ...
  {#if show}
    <div class="fixed inset-0 z-50 flex items-center justify-center" transition:fade={{ duration: 200 }}>
      ...
      <!-- Modal -->
      <div 
        class="modal-content relative w-full max-w-md rounded-xl bg-neutral-900 border border-neutral-800 p-6 shadow-xl" 
        transition:fly={{ y: 20, duration: 200 }}
      >
        <button 
          class="absolute right-4 top-4 rounded-full p-1 text-neutral-400 hover:bg-neutral-800 hover:text-white transition-colors" 
          on:click={closeModal}
        >
          <X size={18} />
        </button>
        ...
        <div class="space-y-4">
          ...
          <button 
            class="w-full flex items-center justify-between p-4 rounded-lg border 
                   {isAppleSilicon ? 'border-primary/50 bg-primary/5' : 'border-neutral-800 bg-neutral-800/50'} 
